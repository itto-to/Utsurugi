...
#define LIGHT_RECEIVABLE_LAYER (CollisionFilter::kPlatform | CollisionFilter::kPlayer | CollisionFilter::kEnemy | CollisionFilter::kIvy)
	...
	void Light::Draw()
	{
		...
		draw_pos += screen_center - Vector3(Camera::main_->transform_->position_, 0.0f) * PIXEL_PER_UNIT + Vector3(-transform_->position_.x,transform_->position_.y,0.0f) / Camera::main_->GetZoom();
	...
	{
		int time_fluc = ((int)Time::Instance()->app_time_ % 60000);	// 時間による明るさのゆらぎ
